...
nimble_code <- nimbleCode({
  ...
  log_mass[1:Time]~dcar_normal(adj=adj[1:(2*Time-2)],num=num[1:Time],tau=psi[1]) 
  #Stickbreaking implementation
  for(i in 1:(m-1)){
    knot1[i] ~ dunif(0,max_ws)
    knot2[i] ~ dunif(0, 360)
    invbw1[i] ~ dgamma(1,lambda) # invbw is the inv bandwidth (smoothing parameter)  
    invbw2[i] ~ dgamma(1,lambda) 
  }
  ...
  #this prior should be changed as it's a leftover and does not have any influence
  lambda ~ dunif(0, lmax) 
...
#create adjacency process for CAR. Just always next neighbour to create AR(1)
#adj includes the indices of the neighbours for each in a single vector
  adj=c(2)
  for(i in 2:(maxtime-1)){
    adj=c(adj,i-1,i+1)
  }
  adj=c(adj,maxtime-1)
  #num includes the number of neighbours for each
  num<-c(1,rep(2,maxtime-2),1)
#now all the constants needed in the nimble code above
model_spec[[3]]<-list("Time"=maxtime,"nS"=length(comps), "m"=maxclust,"alpha"=rep(0.5,length(comps)),
               adj=adj,num=num,"wd"=data.wind$wd[1:maxtime], 
               "ws"=data.wind$ws[1:maxtime],"max_ws"=max(data.wind$ws,na.rm=T) ,lmax=365)
...
model_spec[[4]]<-list(Var=rep(1,length(comps)),prec=1,eps=matrix(0,nrow=maxtime,ncol=maxclust-1),log_mass=as.vector(matrix(0,nrow=maxtime,ncol=1)))
...
conf$addMonitors("p", "mu.Sp","Var", "mu.theta","log_mass","prec","psi","knot1","knot2","invbw1","invbw2")
...
samples <- runMCMC(cMCMC, niter=200000, nburnin=50000, thin=100, nchains=1,samplesAsCodaMCMC = T,summary=T)
...
save(samples,maxclust,maxtime,comps,S_prep,file=paste0("samples/","nimble_c",maxclust,"-","_T_",maxtime,"_comps_",min(comps),"-",max(comps),"_",Sys.Date(),".RData"))
...
ggs_traceplot(S_split%>%filter(Parameter%in%paste0("p[3, ",c(29,22,17,18),"]")))
