...
clust_comp<-function(maxclust,maxtime,comps,S_prep,clustfilt=1:maxclust,sizegroup){
  ...
  plt1c<-ggplot(z%>%filter(source%in%clustfilt))+geom_line(aes(x=size.center,y=mean,group=source,colour=source))+geom_ribbon(aes(x=size.center,ymin=low,ymax=high,group=source,fill=source),alpha=0.5)+
    scale_colour_viridis_d()+scale_x_log10()
  ...
  plt2<-ggplot(z3%>%filter(source%in%clustfilt))+geom_tile(aes(x=wdgrid,y=wsgrid,fill=kernel))+scale_fill_viridis_c(option = "magma",limits=c(0,1))+
    coord_polar() +geom_point(aes(x=0,y=0),size=2,colour="red")+facet_wrap(~source)
...
Comp_summary<-function(maxclust,agg_means,cl_comp,clustprobs){
  ...
  prof<-ggplot(z%>%filter(source==i))+geom_pointrange(aes(x=size.center,y=mean,ymin=low,ymax=high))+scale_x_log10()
  prof_log<-ggplot(z%>%filter(source==i))+geom_pointrange(aes(x=size.center,y=mean,ymin=low,ymax=high))+ coord_trans(y = logit_perc)+scale_y_continuous(breaks=c(0.0001,0.001,0.01,0.5,0.1,0.2))+scale_x_log10()
  wind<-ggplot(z3%>%filter(source==i))+geom_tile(aes(x=wdgrid,y=wsgrid,fill=kernel))+scale_fill_viridis_c(option = "magma",limits=c(0,1))+
    coord_polar() +geom_point(aes(x=0,y=0),size=2,colour="red")
...
Total_summary<-function(maxclust,agg_means,clustprobs,S_prep,sizegroup){
  ...
  f2<-clustprobs[[4]]
  week<-ggplot(c1%>%filter(source_n=="Total"))+geom_pointrange(aes(x=weekday,y=m,ymin=low,ymax=high))+scale_y_log10()
  month<-ggplot(c2%>%filter(source_n=="Total"))+geom_pointrange(aes(x=month,y=m,ymin=low,ymax=high))+scale_y_log10()
  day<-ggplot(c3%>%filter(source_n=="Total"))+geom_pointrange(aes(x=center_hour,y=m,ymin=low,ymax=high))+scale_y_log10()
  barp<-ggplot(f1, aes(x=as.factor(source),y=p)) + 
    geom_bar(stat="identity")+labs(y="Mean prop. of conc.",x="Source")
  barc<-ggplot(f1, aes(x=as.factor(source),y=c)) + 
    geom_bar(stat="identity")+labs(y="Mean conc.",x="Source")
  conc<-ggplot(f2)+geom_line(aes(x=time,y=mean))+geom_ribbon(aes(x=time,ymin=low,ymax=high),colour="grey",alpha=0.5)+scale_y_log10()
  sds<-ggplot(v)+geom_pointrange(aes(x=size,y=sqrt(mean),ymin=sqrt(low),ymax=sqrt(high)))+labs(y="Standard deviation",x="Size")
  plt<-ggarrange(   # plot4 in first row
    ggarrange(barp,barc,sds,ncol=3),
    ggarrange(week,month,day, ncol = 3), 
    conc,
    nrow = 3  # plot1 and plot2 in second row
  )
  return(plt)
