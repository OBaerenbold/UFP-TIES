...
pollutants_corr<-function(data,poll,maxtime,maxclust){
  library(corrplot)
  probs<-data%>%pivot_wider(id_cols = c("time","source"),names_from="source",names_prefix = "source_",values_from="p_mean")%>%select(-time)
  cons<-data%>%pivot_wider(id_cols = c("time","source"),names_from="source",names_prefix = "source_",values_from="c_mean")%>%select(-time)
  ...
  pdta$type1<-levels
  pdta<-pdta%>%pivot_longer(`source_1`:PMFR,names_to="type2",values_to="cor")
  pdta$type1<-factor(pdta$type1,levels=levels)
  pdta$type2<-factor(pdta$type2,levels=levels)
  plt1<-ggplot(pdta%>%filter(type1%in%levels[1:maxclust],type2%in%levels[1:maxclust]))+geom_tile(aes(x=type1,y=type2,fill=cor))+scale_fill_viridis_c()+ geom_text(aes(x=type1,y=type2,label = round(cor, 2)),colour="white") + 
  ...
  plt2<-ggplot(pdta%>%filter(type1%in%levels[(maxclust+1):length(levels)],type2%in%levels[1:maxclust]))+geom_tile(aes(x=type1,y=type2,fill=cor))+scale_fill_viridis_c()+ geom_text(aes(x=type1,y=type2,label = round(cor, 2)),colour="white")+
        ylab('Source')+xlab('') + labs(fill="Correlation") + scale_y_discrete(labels=1:maxclust)
  plt3<-ggplot(pdta%>%filter(type1%in%levels[(maxclust+1):length(levels)],type2%in%levels[(maxclust+1):length(levels)]))+geom_tile(aes(x=type1,y=type2,fill=cor))+scale_fill_viridis_c()+ geom_text(aes(x=type1,y=type2,label = round(cor, 2)),colour="white")+
        ylab('')+xlab('') + labs(fill="Correlation") 
  #same for concentrations
  cdta<-as.data.frame(cdta)
  cdta$type1<-levels
  cdta<-cdta%>%pivot_longer(`source_1`:PMFR,names_to="type2",values_to="cor")
...
clust_comp<-function(maxclust,maxtime,comps,S_prep,clustfilt=1:maxclust,sizegroup){
  ...
  plt1c<-ggplot(z%>%filter(source%in%clustfilt))+geom_line(aes(x=size.center,y=mean,group=source,colour=source))+geom_ribbon(aes(x=size.center,ymin=low,ymax=high,group=source,fill=source),alpha=0.5)+
    scale_colour_viridis_d()+scale_x_log10()+xlab('Particle Size (nm)')+ ylab('')+ggtitle('Source profile') +labs(fill='Source',colour='Source')
  ...
  plt2<-ggplot(z3%>%filter(source%in%clustfilt))+geom_tile(aes(x=wdgrid,y=wsgrid,fill=kernel))+scale_fill_viridis_c(option = "magma",limits=c(0,1))+
    coord_polar(start = -5 / 180 * pi) +geom_point(aes(x=0,y=0),size=2,colour="red")+facet_wrap(~source)+ylab('Wind Speed')+xlab('Wind direction')+labs(fill='Kernel')+
...
Comp_summary<-function(maxclust,agg_means,cl_comp,clustprobs){
  z<-cl_comp[[1]][[1]]
  z3<-cl_comp[[2]][[1]]
  c1<-agg_means[[1]]
  c2<-agg_means[[2]]
  ...
  t<-list()
  for(i in 1:maxclust){
  prop<-ggplot(f%>%filter(source==i))+geom_line(aes(x=time,y=p_mean))+geom_ribbon(aes(x=time,ymin=p_low,ymax=p_high),colour="grey",alpha=0.5)
  conc<-ggplot(f%>%filter(source==i))+geom_line(aes(x=time,y=c_mean))+geom_ribbon(aes(x=time,ymin=c_low,ymax=c_high),colour="grey",alpha=0.5)+scale_y_log10()
  prof<-ggplot(z%>%filter(source==i))+geom_pointrange(aes(x=size.center,y=mean,ymin=low,ymax=high))+scale_x_log10()+xlab('Particle Size (nm)')+ ylab('Profile')
  prof_log<-ggplot(z%>%filter(source==i))+geom_pointrange(aes(x=size.center,y=mean,ymin=low,ymax=high))+ 
    coord_trans(y = logit_perc)+scale_y_continuous(breaks=c(0.0001,0.001,0.01,0.5,0.1,0.2))+scale_x_log10()+xlab('Particle Size (nm)')+ ylab('Log profile')
  wind<-ggplot(z3%>%filter(source==i))+geom_tile(aes(x=wdgrid,y=wsgrid,fill=kernel))+scale_fill_viridis_c(option = "magma",limits=c(0,1))+
    coord_polar(start = -5 / 180 * pi) +geom_point(aes(x=0,y=0),size=2,colour="red")+ylab('Wind Speed')+xlab('Wind direction')+
    labs(fill='Kernel')+scale_x_continuous(breaks = c(0,90,180,270)) 
  week<-ggplot(c1%>%filter(source==i))+geom_pointrange(aes(x=weekday,y=m,ymin=low,ymax=high))+scale_y_log10()
  month<-ggplot(c2%>%filter(source==i))+geom_pointrange(aes(x=month,y=m,ymin=low,ymax=high))+scale_y_log10()
  day<-ggplot(c3%>%filter(source==i))+geom_pointrange(aes(x=center_hour,y=m,ymin=low,ymax=high))+scale_y_log10()
  t[[i]]<-annotate_figure(ggarrange(   # plot4 in first row
    ggarrange(wind, week,month,day, ncol = 4), 
    ggarrange(prop,conc,ncol=2),
    ggarrange(prof,prof_log,ncol=2),
    nrow = 3  # plot1 and plot2 in second row
  ),top=paste0("Source - ",i))
...
Total_summary<-function(maxclust,agg_means,clustprobs,S_prep,sizegroup,timeref){
  ...
  c3<-agg_means[[3]]
  f1<-clustprobs[[1]]
  f1<-f1%>%group_by(source)%>%summarise(p=mean(p_mean),c=mean(c_mean))
  f2<-left_join(clustprobs[[4]],timeref, by = c("time"="timeindex"))
  week<-ggplot(c1%>%filter(source_n=="Total"))+geom_pointrange(aes(x=weekday,y=m,ymin=low,ymax=high))+scale_y_log10()+xlab('Weekday')+ylab('c(t)')+ theme(axis.title.y = element_text(angle=0))
  month<-ggplot(c2%>%filter(source_n=="Total"))+geom_pointrange(aes(x=month,y=m,ymin=low,ymax=high))+scale_y_log10()+xlab('Month')+ylab('c(t)')+ theme(axis.title.y = element_text(angle=0))
  day<-ggplot(c3%>%filter(source_n=="Total"))+geom_pointrange(aes(x=center_hour,y=m,ymin=low,ymax=high))+scale_y_log10()+xlab('hour')+ylab('c(t)')+ theme(axis.title.y = element_text(angle=0))
  barp<-ggplot(f1, aes(x=as.factor(source),y=p)) + 
    geom_bar(stat="identity")+labs(y=expression(s[k]),x="Source")+ theme(axis.title.y = element_text(angle=0))
  barc<-ggplot(f1, aes(x=as.factor(source),y=c)) + 
    geom_bar(stat="identity")+labs(y=expression(f[k]),x="Source")+ theme(axis.title.y = element_text(angle=0))
  #conc<-ggplot(f2)+geom_line(aes(x=time,y=mean))+geom_ribbon(aes(x=time,ymin=low,ymax=high),colour="grey",alpha=0.5)+scale_y_log10()
  conc<-ggplot(f2)+geom_line(aes(x=day,y=mean,color=factor(center_hour)))+scale_y_log10()+
    labs(title='Daily mean total particle concentration',color='Hour')+xlab('')+ylab('c(t)')+ theme(axis.title.y = element_text(angle=0))
  sds<-ggplot(v)+geom_pointrange(aes(x=center,y=sqrt(mean),ymin=sqrt(low),ymax=sqrt(high)))+labs(y=expression(sigma[p]),x="Particle Size (nm)")+
  ...
  plt<-ggarrange(   # plot4 in first row
    ...
    annotate_figure(ggarrange(barp,barc,sds,ncol=3),
              top = text_grob("Temporal mean", size = 12,hjust = 2.5)),
    nrow = 3  # plot1 and plot2 in second row
  )
  return(plt)
}
...
#Edits by Carolina E Feb 23, 2022
#Change clust label by source and Cluster by Source, maxclust will remain equal
# add  ylab('Source')+xlab('Source') + labs(fill="Correlation") + scale_x_discrete(labels=1:maxclust) + scale_y_discrete(labels=1:maxclust) to
# plt1,plt4 in pollutants_corr function
# add ylab('Source')+xlab('') + labs(fill="Correlation") + scale_y_discrete(labels=1:maxclust)
# plt2,plt5 in pollutants_corr function
# add ylab('')+xlab('') + labs(fill="Correlation") 
# plt3,plt6 in pollutants_corr function
#Edits by Carolina E Feb 24, 2022
#add xlab('Particle Size (nm)')+ ylab('')+ggtitle('Source profile') + labs(fill='Source',colour='Source') to
#  plt1c in clust_comp
# add coord_polar(start = -5 / 180 * pi) to start alignned to zero (IMPORTANT)
# +ylab('Wind Speed')+xlab('Wind direction')+labs(fill='Kernel')+ 
#  scale_x_continuous(breaks = c(0,90,180,270),labels = c("N", "E", "S","W"))  
# plt2 in clust_comp and wind in Comp_summary
# add xlab('Particle Size (nm)')+ ylab('Profile') to 
#prof and   prof_log accordingly
#Edits by Carolina E Feb 25, 2022
# add timeref as input to Total_summary
# Mayor change in conc of Total_summary
#f2<-left_join(clustprobs[[4]],timeref, by = c("time"="timeindex"))
#ggplot(f2)+geom_line(aes(x=day,y=mean,color=factor(center_hour)))+scale_y_log10()+
#  labs(title='Daily total particle concentration',color='Hour')+xlab('')+ylab('c(t)')
# xlab('Weekday')+ylab('') in day, week, month of Total_summary
# rotate ylab in barp,barc,sds of Total_summary
