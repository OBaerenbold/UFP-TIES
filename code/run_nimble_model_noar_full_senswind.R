...
save(samples,maxclust,maxtime,comps,S_prep,file=paste0("samples/","nimble_noar_s1_c",maxclust,"-","_T_",maxtime,"_comps_",min(comps),"-",max(comps),"_",Sys.Date(),".RData"))
...
ggs_traceplot(S_split%>%filter(Parameter%in%paste0("mu.theta[8, ",11:14,"]")))
ggs_traceplot(S_split,family="Var")
ggs_geweke(S_split,family="Var")
ggs_geweke(S_split,family="mu.theta")
ggs_traceplot(S_split%>%filter(Parameter=="mu.theta[8, 706]"))
