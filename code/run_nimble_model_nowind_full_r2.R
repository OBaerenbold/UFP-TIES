...
conf$addMonitors("p", "mu.Sp","Var", "mu.theta","log_mass","mean_mass","sd_mass","prec")
...
save(samples,maxclust,maxtime,comps,S_prep,file=paste0("samples/","nimble_nowind_r2_c",maxclust,"-","_T_",maxtime,"_comps_",min(comps),"-",max(comps),"_",Sys.Date(),".RData"))
...
ggs_traceplot(S_split%>%filter(Parameter%in%paste0("p[5, ",c(706),"]")))
...
ggs_traceplot(S_split%>%filter(Parameter=="knot2[2]"))
