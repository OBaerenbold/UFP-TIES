...
samples <- runMCMC(cMCMC, niter=100000, nburnin=50000, thin=50, nchains=1,samplesAsCodaMCMC = T,summary=T)
...
ggs_traceplot(S_split,family="knot2")
ggs_traceplot(S_split,family="knot1")
ggs_traceplot(S_split,family="invbw2")
ggs_traceplot(S_split,family="prec")

ggs_traceplot(S_split,family="invbw1")
...
ggs_crosscorrelation(S_split%>%filter(Parameter%in%c(paste0("knot1[",1:4,"]"),paste0("invbw1[",1:4,"]"))))

ggs_traceplot(S_split,family="invbw")
