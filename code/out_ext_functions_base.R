...
clust_comp_gen<-function(maxclust,maxtime,comps,S_prep,clustfilt=1:maxclust,sizegroup,names){
  z<-S_prep%>%filter(Parameter=="mu.theta")%>%group_by(index1,index2)%>%summarise(mean=mean(value),low=quantile(value,c(0.025)),high=quantile(value,c(0.975)))
  z<-z%>%rename("source"="index2","size"="index1")
  z<-z%>%mutate(size.center=as.factor(size))
  levels(z$size.center)<-levels(sizegroup$center)[comps]
  ...
  plt1b<-ggplot(z)+geom_pointrange(aes(x=size.center,y=logit(mean),ymin=logit(low),ymax=logit(high)))+facet_wrap(~source)
  plt1c<-ggplot(z%>%filter(source%in%clustfilt))+
    geom_line(aes(x=size.center,y=mean,group=source_name,colour=source_name))+
    scale_colour_viridis_d()+
    geom_ribbon(aes(x=size.center,ymin=low,ymax=high,group=source_name,fill=source_name),alpha=0.4)+
    scale_fill_viridis_d()+scale_x_log10()+xlab('Particle Size (nm)')+ 
    ylab('Proportion of particle conc.')+ggtitle('Source profile') +labs(fill='Source',colour='Source')+
    guides(linetype = "none")
  z2<-S_prep%>%filter(Parameter%in%c("invbw1","invbw2","knot1","knot2"))%>%group_by(index1,Parameter)%>%summarise(mean=mean(value),low=quantile(value,c(0.025)),high=quantile(value,c(0.975)))
  z2<-z2%>%rename("source"="index1")
...
clust_comp_nowind<-function(maxclust,maxtime,comps,S_prep,clustfilt=1:maxclust,sizegroup,names){
  z<-S_prep%>%filter(Parameter=="mu.theta")%>%group_by(index1,index2)%>%summarise(mean=mean(value),low=quantile(value,c(0.025)),high=quantile(value,c(0.975)))
  z<-z%>%rename("source"="index2","size"="index1")
  z<-z%>%mutate(size.center=as.factor(size))
  levels(z$size.center)<-levels(sizegroup$center)[comps]
...
Comp_summary_nowind<-function(maxclust,agg_means,cl_comp,clustprobs,pollcorr){
  z<-cl_comp[[1]][[1]]
  c1<-agg_means[[1]]
  c2<-agg_means[[2]]
  c3<-agg_means[[3]]
